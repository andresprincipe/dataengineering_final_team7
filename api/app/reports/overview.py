...
def overview(
    ...
    queries = [
        ...
        SELECT
            o.county,
            o.year::int AS year,
            o.total_enforcements::int AS total_enforcements,
            o.average_wage::float AS average_wage
        ...
        WITH enforcement_counts AS (
            ...
            GROUP BY COALESCE(c.county_name, e.county),
                     COALESCE(EXTRACT(YEAR FROM e.action_date)::int, e.year::int)
        ),
        wages_norm AS (
            SELECT
